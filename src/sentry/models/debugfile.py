...
class ProjectCacheFile(Model):
    ...
    debug_file = FlexibleForeignKey('sentry.ProjectDebugFile', rel_class=OneToOneRel)
...
# XXX: This field was originally named dsym_file and needs to be renamed for
# backwards compatibility. Django 1.10 would allow us to override field from
# abstract base classes directly in the class declaration.
ProjectSymCacheFile._meta.get_field('debug_file').db_column = 'dsym_file_id'
...
class DIFCache(object):
    ...
    def _get_caches_impl(self, project, debug_ids, cls, on_dif_referenced=None):
        ...
        debug_ids = [six.text_type(debug_id).lower() for debug_id in debug_ids]
        debug_files = find_debug_files(project, debug_ids, features=cls.required_features)
